...
var MssqlDriver = Base.extend({
    ...
    addMigrationRecord: function(name, callback) {
        ...
        return this.runSql(
            `INSERT INTO [${internals.migrationTable}] ([name], [run_on]) VALUES ('${name}', '${formattedDate}')`
        )
    ...
    all: function(sql) {
        ...
        return new Promise(function(resolve, reject) {
            return self.connection
                ...
                .then(function(queryResult) {
                    return resolve(
                        queryResult.recordset ? queryResult.recordset : []
                    );
                })
                .catch(function(err) {
                    return reject(err);
    ...
    allLoadedMigrations: function(callback) {
        ...
        return this.all(sql)
            ...
            .catch(function(err) {
                return callback(err, null); // figureout callback params
            });
    },
    ...
    createTable: function(tableName, options, callback) {
        ...
        var sql = `${ifNotExistsSql} CREATE TABLE [${tableName}] (${columnDefs.join(
            ', '
        )}${pkSql}) ${ifNotExistsSqlEnd}`;
